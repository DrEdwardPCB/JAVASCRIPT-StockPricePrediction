const { SimpleMovingAverage, WeightedMovingAverage, ExpotentialMovingAverage } = require('../technicalAnalysis/movingAverage')
const { bollingerBand } = require('../technicalAnalysis/BollingerBand')
const { MACD } = require('../technicalAnalysis/MACD')
const { RSI, CulterRSI } = require('../technicalAnalysis/RSI')
 ...
 * @param {number} window
 */
exports.train = async (data, callback, offset = offsetC, window = windowC) => {
    /*
    tfvis.visor()
    tf.setBackend('webgl')
    ...
    //Technical analysis
    const [s10arr, haha29] = minmaxnormalize(SimpleMovingAverage(NpArray, 10).slice(250))
    const [s20arr, haha28] = minmaxnormalize(SimpleMovingAverage(NpArray, 20).slice(250))
    const [s50arr, haha27] = minmaxnormalize(SimpleMovingAverage(NpArray, 50).slice(250))
    const [s100arr, haha26] = minmaxnormalize(SimpleMovingAverage(NpArray, 100).slice(250))
    const [s200arr, haha25] = minmaxnormalize(SimpleMovingAverage(NpArray, 200).slice(250))
    const [s250arr, haha24] = minmaxnormalize(SimpleMovingAverage(NpArray, 250).slice(250))
    const [w10arr, haha23] = minmaxnormalize(WeightedMovingAverage(NpArray, 10).slice(250))
    const [w20arr, haha22] = minmaxnormalize(WeightedMovingAverage(NpArray, 20).slice(250))
    const [w50arr, haha21] = minmaxnormalize(WeightedMovingAverage(NpArray, 50).slice(250))
    const [w100arr, haha20] = minmaxnormalize(WeightedMovingAverage(NpArray, 100).slice(250))
    const [w200arr, haha19] = minmaxnormalize(WeightedMovingAverage(NpArray, 200).slice(250))
    const [w250arr, haha18] = minmaxnormalize(WeightedMovingAverage(NpArray, 250).slice(250))
    const [e10arr, haha17] = minmaxnormalize(ExpotentialMovingAverage(NpArray, 10, 2).slice(250))
    const [e20arr, haha16] = minmaxnormalize(ExpotentialMovingAverage(NpArray, 20, 2).slice(250))
    const [e50arr, haha15] = minmaxnormalize(ExpotentialMovingAverage(NpArray, 50, 2).slice(250))
    const [e100arr, haha14] = minmaxnormalize(ExpotentialMovingAverage(NpArray, 100, 2).slice(250))
    const [e200arr, haha13] = minmaxnormalize(ExpotentialMovingAverage(NpArray, 200, 2).slice(250))
    const [e250arr, haha12] = minmaxnormalize(ExpotentialMovingAverage(NpArray, 250, 2).slice(250))
    var [bolu1, bold1] = bollingerBand(NpArray)
    const [bolu, haha11] = minmaxnormalize(bolu1.slice(250))
    const [bold, haha10] = minmaxnormalize(bold1.slice(250))
    const [macd, haha9] = minmaxnormalize(MACD(NpArray).slice(250))
    const [rsi, haha8] = minmaxnormalize(RSI(NpArray).slice(250))
    const [crsi, haha7] = minmaxnormalize(CulterRSI(NpArray).slice(250))
    const [so, hah6a] = minmaxnormalize(StochasticOscillator(NpArray).slice(250))
    const [roc, haha5] = minmaxnormalize(ROC(NpArray).slice(250))
    const [mfi, haha4] = minmaxnormalize(MFI(NpArray).slice(250))

    const [tNparr, denorfncNparr] = minmaxnormalize(NpArray.slice(250))
    const [tHarr, haha1] = minmaxnormalize(HighArray.slice(250))
    const [tLarr, haha2] = minmaxnormalize(LowArray.slice(250))
    const [tPCarr, haha3] = minmaxnormalize(PCArray.slice(250))
    ...
    Xs = _.takeRight(Xs, Xs.length - window - offset - 1)
    ...
    callback.emit('vis', { type: 'model', data: model.summary() })
    model.compile({
        optimizer: optimizer
    ...
    */
    ...
    await model.fit(realXs, realYs, {
        ...
        callbacks: {
            onEpochEnd: async (epoch, logs) => {
                callback.emit('vis', { type: 'history', data: logs })
            }
        },
    ...
    const last = [
        tf.tensor(answer.map(e => {//there will be 200 time step 30 feature for each
            return e.map((f, inde) => {
                return [
                    f[0], f[1], f[2], f[3],
                ]
    ...
    const predsArrayNor = denorfncNparr(predsArray.map(e => e[0]))
    console.log(predsArray)
    const answerReturn = denorfncNparr(answer.map((e, i, arr) => {//there will be 200 time step 30 feature for each
        //console.log(e.length)
        if (i < arr.length-offset-1) {
            return e.map((f) => { return f[30] }).filter((f, inde, rarr) => { return inde == rarr.length - 1 })[0]
        }else{
            return 0
        }
    }))
    ...
    callback.emit('result', {
        answer: answerReturn, predict: predsArrayNor
    })
